...
const Header = () => {
  ...
  return (
    <nav className="navbar navbar-expand-xl navbar-light sticky-top bg-light py-3 shadow-sm">
      <div className="container-fluid mx-auto">
        ...
        <div
          className="container collapse navbar-collapse justify-content-center"
          id="appHeader"
        >
          {navItems.map(({ text, route }, idx) => {
            return (
              <ul
                className="navbar-nav mb-2 mb-lg-0 mx-5 px-2 header__ul rounded"
                key={idx}
              >
                <li className="nav-item">
                  <NavLink
                    className={`nav-link ${
                      location.pathname === route && 'disabled'
                    }`}
                    to={route}
                  >
                    {text}
